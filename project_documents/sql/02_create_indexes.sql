...
CREATE INDEX IF NOT EXISTS idx_orders_delivered_cov 
    ON sales.orders(order_status, order_date DESC) 
    INCLUDE (net_total, cust_id, store_id);
...
CREATE INDEX IF NOT EXISTS idx_order_items_order_cov 
    ON sales.order_items(order_id) 
    INCLUDE (prod_id, quantity, net_amount, unit_price);
